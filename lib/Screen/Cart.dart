...
String? tidds;
  ...
  tidds = prefs.getString('Scanner_table_id');
  ...
  Future<void> _getCart(String save) async {
    ...
    if (_isNetworkAvail) {
      try {
        ...
        print("checking cart api and para ${getCartApi} and ${parameter}");
        Response response =
        await post(getCartApi, body: parameter, headers: headers)
            .timeout(Duration(seconds: timeOut));
        print(getCartApi.toString());
                                ...
                                SingleChildScrollView(
                                  child: Padding(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                       // address(),
                                        isTakeway == false ? SizedBox.shrink() :  payment(),
                                        cartItems(cartList),
                                       // / promo(),
                                        orderSummary(cartList),
  ...
  razorpayPayment() async {
    ...
    String? contact = settingsProvider.mobile;
    String? email = settingsProvider.email;
      print("${contact} and ${email}  both are here");
    String amt = ((totalPrice) * 100).toStringAsFixed(2);
      print("checking amount here ${amt} and ${razorpayId}");
    if (contact != '' && email != '') {
      context.read<CartProvider>().setProgress(true);
      // checkoutState!(() {});
      var options = {
  ...
  Future<void> placeOrder(String? tranId) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String tableids = prefs.getString('newTableId').toString();
    String tid = prefs.getString('Scanner_table_id').toString();
    print("sdsdsdssdsdsdsdsds ${tidds}  sdsfdfs ${tid}");
    // String sId = prefs.getString('seller_id')!;
    ...
    if (_isNetworkAvail) {
      ...
      try {
        ...
        var parameter = {
         ...
         // 'table_no': isTakeway == true ? "0" : "${tidds}",
          'order_date':'${currentDate}',
       ...
       //   ADD_ID: selAddress,
        ...
        // }
        if(isTakeway == true){
        parameter['table_no'] = "0";
        }
        else{
          parameter['table_no'] = "${tid}";
        }
